...
class AvailableEquipmentPage extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<AvailableEquipmentPage> createState() => _AvailableEquipmentPageState();
}

class _AvailableEquipmentPageState extends ConsumerState<AvailableEquipmentPage> {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        ...
        Expanded(
          child: ref.watch(equipmentListProvider(_searchQuery)).when(
                data: (items) => Stack(
                  children: [
                    SingleChildScrollView(
                      child: items.isNotEmpty
                          ? Column(
                              children: items.map((item) => EquipmentCard(item)).toList(),
                            )
                          : Padding(
                              padding: const EdgeInsets.all(20),
                              child: Text('Trenutno nema rezultata za ovu pretragu.'),
                            ),
                    ),
                    Positioned(
                      bottom: 20,
